// https://github.com/Cyanilux/URP_ShaderCodeTemplates/blob/main/PBRSurface.hlsl
...
void InitializeSurfaceData(Varyings IN, out SurfaceData surfaceData){
	...
	surfaceData.alpha = 1.0;//Alpha(albedoAlpha.a, _BaseColor, _Cutoff);
	// surfaceData.albedo = albedoAlpha.rgb * _BaseColor.rgb * IN.color.rgb;
	surfaceData.albedo = lerp(_FootColor, _TipColor, IN.uv.y);

	surfaceData.normalTS = SampleNormal(IN.uv, TEXTURE2D_ARGS(_BumpMap, sampler_BumpMap), _BumpScale);
	surfaceData.emission = 0.0;//SampleEmission(IN.uv, _EmissionColor.rgb, TEXTURE2D_ARGS(_EmissionMap, sampler_EmissionMap));
	surfaceData.occlusion = 1.0;//SampleOcclusion(IN.uv);
